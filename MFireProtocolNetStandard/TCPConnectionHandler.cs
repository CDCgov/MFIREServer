...
namespace MFireProtocol
{
    public abstract class TCPConnectionHandler : IDisposable
    {
        ...
        private bool SendPacket(byte[] pdata, Socket s, byte[] sendBuffer, MemoryStream sendBufferStream, BinaryWriter sendBufferWriter)
        {
            if (_hasDisconnected)
                return false;

            UInt32 packetSize = (UInt32)pdata.Length;

            sendBufferStream.Seek(0, SeekOrigin.Begin);
            sendBufferWriter.Write(packetSize);
            sendBufferWriter.Write(pdata);
        ...
        protected abstract void DecodePacket(Stream s);
        ...
        private void RaisePacketReceived()
        {
